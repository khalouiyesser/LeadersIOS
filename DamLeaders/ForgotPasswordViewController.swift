...
class ForgotPasswordViewController: UIViewController,UITextFieldDelegate {
    ...
    @IBAction func continueAction(_ sender: Any) {
        ...
        print("hello")
        guard let emailText = otpMail.text,
              let smsText = otpSMS.text else {
    ...
    */
    ...
    // Sends the signup request to the server
       private func sendSignupRequest(parameters: [String: Any]) {
           guard let url = URL(string: "http://192.168.137.70:3000/auth/forgot-password") else { return }
           do {
                          ...
                          /* if let statusCode = jsonResponse["statusCode"] as? Int {
                               ...
                               //    if  let yesser = try JSONDecoder().decode(ForgotPasswordData.self, from: jsonData){
                                   if let code = jsonResponse["code"] as? Int ,
                                      let restToken = jsonResponse["restToken"] as? String {
                                       
                                       print("---------------------------------------")
                                       DispatchQueue.main.async {
                                           self.performSegue(withIdentifier: "otp", sender: (restToken ,code))
                                       }
                                   }
    ...
    */
    ...
    private func handleResponse(data: Data) {
        struct ForgotPasswordData: Decodable {
            let statusCode: Int
            let code: Int
            let resetToken: String  // Assurez-vous que c'est bien `resetToken` (en tenant compte du nom exact)
        }
        ...
        if let responseString = String(data: data, encoding: .utf8) {
            ...
            do {
                // Essayer de décoder le JSON en une structure ForgotPasswordData
                let decodedResponse = try JSONDecoder().decode(ForgotPasswordData.self, from: data)
                print(decodedResponse)
                
                // Extraire les valeurs du statut et des autres données
                let statusCode = decodedResponse.statusCode
                let code = decodedResponse.code
                let resetToken = decodedResponse.resetToken
                
                // Vérifier le statut de la réponse (par exemple, code 200 pour succès)
                if statusCode == 200 {
                    print("Opération réussie avec code \(code) et resetToken \(resetToken)")
                    
                    // Si la réponse est réussie, effectuer la transition vers la vue suivante
                    DispatchQueue.main.async {
                        self.performSegue(withIdentifier: "otp", sender: (resetToken, code))
                    }
                } else {
                    // Gérer les erreurs en fonction du statut du code
                    print("Erreur de statut : \(statusCode)")
                    DispatchQueue.main.async {
                        self.showAlert(message: "Erreur avec le code de statut: \(statusCode).")
                    }
                }
            ...
            DispatchQueue.main.async {
                self.showAlert(message: "Erreur: Impossible de traiter la réponse du serveur.")
            }
        }
    }
