...
class LoginViewController: UIViewController, UITextFieldDelegate  {
    ...
    // Variables pour stocker le token et l'ID utilisateur
    ...
    /*private func handleResponse(data: Data) {
        if let responseString = String(data: data, encoding: .utf8) {
            ...
            // Tentative de parsing de la réponse en JSON
            if let jsonData = responseString.data(using: .utf8) {
                do {
                    if let jsonResponse = try JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any] {
                        
                        // Extraire les informations de la réponse
                        if let token = jsonResponse["accessToken"] as? String {
                            accessToken = token // Assigner le token à la variable globale
                        }
                        
                        if let id = jsonResponse["userId"] as? String {
                            userId = id // Assigner l'ID utilisateur à la variable globale
                        }
                        
                        // Vérification du code de statut
                        if let statusCode = jsonResponse["statusCode"] as? Int, statusCode == 200 {
                            // Poursuivre avec la logique de navigation
                            ...
                            // Gérer le cas où statusCode n'est pas 200
                            print("Erreur de statut: \(jsonResponse["statusCode"] ?? "inconnu")")
                            DispatchQueue.main.async {
                                self.showAlert(message: "Erreur de statut: \(jsonResponse["statusCode"] ?? "inconnu").")
                            }
                        }
                    }
                } catch {
                ...
                do {
                    if let jsonResponse = try JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any] {
                        ...
                        if let statusCode = jsonResponse["statusCode"] as? Int {
                            ...
                            case 401:
                                // Incorrect credentials (e.g., invalid username or password)
                                DispatchQueue.main.async {
                                    self.showAlert(message: "Identifiants incorrects. Veuillez vérifier votre nom d'utilisateur et votre mot de passe.")
                                }
                            case 400:
                                // Incorrect credentials (e.g., invalid username or password)
                                DispatchQueue.main.async {
                            ...
                            case 503:
                                // Service unavailable (e.g., server down or no connection)
                                DispatchQueue.main.async {
                                    self.showAlert(message: "Le service est temporairement indisponible. Vérifiez votre connexion et réessayez.")
                                }
                            case 500:
                                // Internal server error
                                DispatchQueue.main.async {
                                    self.showAlert(message: "Erreur interne du serveur. Veuillez réessayer plus tard.")
                                }
                            default:
                                // Handle other status codes (e.g., 400, 404, etc.)
                                DispatchQueue.main.async {
                                    self.showAlert(message: "Erreur inconnue: \(statusCode). Veuillez réessayer plus tard.")
                                }
                            }
                        } else {
                            // If statusCode is missing or invalid
                            DispatchQueue.main.async {
                                self.showAlert(message: "Code de statut manquant ou invalide.")
                            }
                        }
                    }
                } catch {
